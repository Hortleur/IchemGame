	...
	let questions = [
		...
		},
		{
			...
			reponses:[
				{
					id: 0,
					value:"Tu sais de quoi tu veux parler et tu t'appuieras sur les questions de ton auditoire."
				},
				{
					id: 1,
...
<section id="presentation" class=" mt-4 mb-10">
	<div class=" lg:flex lg:flex-row lg:flex-nowrap lg:justify-evenly lg:items-center lg:w-8/12 lg:mx-auto">
		<div class=" lg:w-1/2 pr-2">
			<p class=" mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				Gagner en compétence vous intéresse ?
			...
			<p class="mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				Ancrer un vrai savoir faire est important pour vous ? Rehaussez votre formation : Libérez le pouvoir de la gamification pour révolutionner l'apprentissage et stimuler l'engagement.
			</p>
			<p class="mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				Même dans le confort de votre maison !
			</p>
			<p class="mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				En présentiel ou en distanciel vous en avez assez des sessions de formation sans intérêt qui échouent à captiver votre public ?
			</p>
			<p class="mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				On vous attend, fort de plus de 20 ans d’expériences dans la formation de formateurs, nous nous sommes fixés comme but de former les meilleurs formateurs du monde francophone grâce à un contenu innovant et une pédagogie unique en francophonie base sur les sciences cognitives. 
			</p>
			<p class="mt-3 first-letter:font-bold first-letter:text-4xl first-letter:text-accent">
				Notre formation étant reconnue recevrez ensuite une certification enregistrée sur France Compétences.
			</p>
		</div>
	...
	>		
		...
		<div>
			<p>Nous vous proposons un petit test formateur facultatif pour une réponse plus personalisée.</p>
			<div class="form-control">
				<label class="label cursor-pointer  justify-center gap-2">
				  ...
				  <input type="checkbox" class="toggle toggle-secondary" on:change={enableTest}/>
				</label>
			</div>
		</div>
		...
		{#if !testOver}
						...
						<div>
							{#each question.reponses as resp }
								<label for="" class=" label cursor-pointer">
									...
									<input type="radio" name="reponses" id="{resp.id}" value="{resp.value}" on:change={handleCheck} class=" radio radio-secondary checked:radio-success">
								</label>
							{/each}
						</div>
					</div>
			...
			<div class=" flex flex-row flex-nowrap justify-center gap-3">
				{#if questionId > 0}
					<button on:click|preventDefault={prevQ} class=" btn btn-secondary">Précédente</button>
				{/if}
				{#if questionId < questions.length - 1}
					<button on:click|preventDefault={nextQ(getQuestion(questionId), reponse)} class=" btn btn-secondary">Suivante</button>
				{/if}
				{#if questionId == questions.length - 1}
					<button on:click|preventDefault={finalQ(getQuestion(questionId), reponse)} class=" btn btn-secondary">Dernière étape</button>
				{/if}
			</div>
		{/if}
		...
		<button class="btn btn-accent text-white font-bold" on:click|preventDefault={send}>
			{#if !loading}
